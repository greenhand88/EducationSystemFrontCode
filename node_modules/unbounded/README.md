...
Function.prototype.bind replacement that provides an `unbounded` hidden property on the returned bounded function, that
contains the original unbounded function
...
This package functions like [`Function.prototype.bind`](https://devdocs.io/javascript/global_objects/function/bind),
however it exposes a hidden `unbounded` property on the returned bounded function that contains the original unbounded
function.

This is very useful as it means you can do `(fn.unbounded || fn).length` to always get the correct amount of arguments,
or `(fn.unbounded || fn).toString()` to always get the source code of any function instead of
getting `function () { [native code] }` for bounded functions.
...
<a href="https://github.com/bevry/unbounded/blob/master/HISTORY.md#files">Discover the release history by heading on
...
<a href="https://github.com/bevry/unbounded/blob/master/CONTRIBUTING.md#files">Discover how you can contribute by
