    ...
    compile: function (opts, next) {
        ...
        steps(next, [
            ...
            function (complete) {
                ...
                console.log('\ncoffee compile:')
                spawn(NODE, [COFFEE, '-co', config.COFFEE_OUT_PATH, config.COFFEE_SRC_PATH], {
                    output: true,
                    cwd: APP_PATH
                }, complete)
            },
            ...
            function (complete) {
                ...
                console.log('\nbabel compile:')
                spawn(NODE, [BABEL, config.BABEL_SRC_PATH, '--out-dir', config.BABEL_OUT_PATH], {
                    output: true,
                    cwd: APP_PATH
                }, complete)
            },
    ...
    watch: function (opts, next) {
        ...
        steps(next, [
            ...
            function (complete) {
                if (!config.BABEL_SRC_PATH || !fsUtil.existsSync(BABEL)) return complete()
                console.log('\nbabel compile:')
                spawn(NODE, [BABEL, '-w', config.BABEL_SRC_PATH, '--out-dir', config.BABEL_OUT_PATH], {
                    output: true,
                    cwd: APP_PATH
                }, complete)
            },
            ...
            function (complete) {
                if (!config.COFFEE_SRC_PATH || !fsUtil.existsSync(COFFEE)) return complete()
                console.log('\ncoffee watch:')
                spawn(NODE, [COFFEE, '-wco', config.COFFEE_OUT_PATH, config.COFFEE_SRC_PATH], {
                    output: true,
                    cwd: APP_PATH
                })  // background
