...
function requireEdition(edition /* :edition */, opts /* :options */) /* :any */ {
    ...
    if (!dir && !entry) {
        const editionFailure = new DetailedError('Skipped edition due to no entry or directory being specified:', {
            edition,
            cwd,
            dir,
            entry
    ...
    if (syntaxes && syntaxCombination) {
        ...
        if (unsupportedSyntaxes.length) {
            const editionFailure = new DetailedError('Skipped edition due to it containing an unsupported syntax:', {
                edition,
                unsupportedSyntaxes
            })
            throw editionFailure
        ...
        // Is this syntax combination unsupported? If so skip it with a soft failure to try the next edition
            ...
            const editionFailure = new DetailedError('Skipped edition due to its syntax combinatiom failing previously:', {
                edition,
    ...
    try {
        ...
        const editionFailure = new DetailedError('Failed to load the edition due to a load error:', {
            edition,
            error: error.stack
        })
